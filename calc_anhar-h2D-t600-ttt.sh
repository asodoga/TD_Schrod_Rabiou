...
# Print nb and nq for logging
echo "nb = $nb, nq = $nq"

# Create results directory and move into it
mkdir -p results_anhar-h2D-t600-ttt_nb$nb
cd results_anhar-h2D-t600-ttt_nb$nb
...
&prop
    t0=0.0 tf=600.0 delta_t=0.1         ! Time range and step
    eps=1.e-20 max_iter=500            ! Precision and max iterations
    propa_name='hagedorn'              ! Primary propagator
    propa_name2='taylor'               ! Optional secondary propagator
    Beta=t P=t renorm= t               ! propagation options
/
**
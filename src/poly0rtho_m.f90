      ...
      USE UtilLib_m
      IMPLICIT NONE
      real(kind=Rk)  :: Funct_1D

       real(kind=Rk), intent(in) :: x
       integer, intent(in)          :: i,first_i,ntyp

       real(kind=Rk) :: fourier,poly_legendre,poly_Hermite,poly_Hermite_exp
...
!================================================================
      ...
      IMPLICIT NONE
       ...
       ELSE
         ...
         d1 = real(ii,                  kind=Rk) * d1
         d2 = real(ii * (ii-1),         kind=Rk) * d2
         d3 = real(ii * (ii-1) * (ii-2),kind=Rk) * d3
       END IF
...
!---------------------------------------------------------------------
       ...
       ii = i/2
       xx = mod(x*real(ii,kind=Rk),pi+pi)
       ...
       ELSE
         ...
         IF (mod(i,2) ==0) THEN
           d0 = s
           d1 =  real(ii,kind=Rk)      * c
           d2 = -real(ii * ii,kind=Rk) * d0
           d3 = -real(ii * ii,kind=Rk) * d1
         ELSE
           d0 =  c
           d1 = -real(ii,kind=Rk)      * s
           d2 = -real(ii * ii,kind=Rk) * d0
           d3 = -real(ii * ii,kind=Rk) * d1
         END IF
       END IF
...
!---------------------------------------------------------------------
       ...
       xx = mod(x*real(i,kind=Rk),pi+pi)
       s = sin(xx) * norme
       c = cos(xx) * norme

       d0 = s
       d1 =  real(i,kind=Rk)     * c
       d2 = -real(i * i,kind=Rk) * d0
       d3 = -real(i * i,kind=Rk) * d1
...
!===================================================
      ...
      USE UtilLib_m
      IMPLICIT NONE
      real(kind=Rk) :: poly_cheby

       real(kind=Rk), intent(in) :: x
       integer, intent(in)          :: nn

       real(kind=Rk) :: p0,p1,p2
       integer :: i,n
...
!===================================================
      ...
      real(kind=Rk), intent(in) :: x(nq)

      real(kind=Rk) :: d0p(nq,nb)

      real(kind=Rk) :: p0(nq),p1(nq),p2(nq)
...
!===================================================
      ...
      real(kind=Rk) :: p0(nq),p1(nq),p2(nq),sqw(nq)
...
!===================================================
      ...
      real(kind=Rk) :: poly_legendre
       ...
       IF ( m == l) THEN
         poly = pmm
       ELSE
         pmmp1 = x*real(2*m+1,kind=Rk)*pmm
         IF (l == m+1) THEN
           poly = pmmp1
         ELSE
           DO ll=m+2,l

             pll = (x*real(2*ll-1,kind=Rk)*pmmp1-real(ll+m-1,kind=Rk)*pmm) / &
                    real(ll-m,kind=Rk)
             pmm=pmmp1
             pmmp1=pll
           END DO
...
!      nomalisation de Pl,m(x)
       norme2 = TWO/real(2*l+1,kind=Rk)
       DO i=l-m+1,l+m
         norme2 = norme2 * real(i,kind=Rk)
       END DO
...
!      write(out_unitp,*) l,m,norme2
       ...
       USE UtilLib_m
       IMPLICIT NONE
       real(kind=Rk) :: UNpoly_legendre

        real(kind=Rk) x,xx

        real(kind=Rk) pmm,somx2,fact,pmmp1,pll,poly
...
!===================================================
      ...
      real(kind=Rk) :: d1poly_legendre
       ...
       ELSE
         d1poly_legendre = real(l,kind=Rk)/(x*x-ONE) * (             &
                    x*poly_legendre(x,lll,0) -                          &
                   sqrt(real(2*l+2,kind=Rk)/real(2*l-1,kind=Rk))* &
      ...
      IMPLICIT NONE
       ...
       ELSE
         ...
         ELSE
           ...
           d1 = real(l,kind=Rk)/(x*x-ONE) * (x*d0 -                  &
           sqrt(real(2*l+1,kind=Rk)/real(2*l-1,kind=Rk))*d01 )

           d2 = (-d0 * real(l*(l+1),kind=Rk) + TWO*x*d1)/(ONE-x*x)
         END IF
       END IF
...
!===================================================
      ...
      IMPLICIT NONE
       ...
       ELSE
         ...
         d2 = (-d0 * real(l*(l+1),kind=Rk) + TWO*x*d1)/(ONE-x*x)
         d3 = (d1 * real(2-l*(l+1),kind=Rk) + d2 * FOUR*x ) / (ONE-x*x)
...
!===================================================
      ...
      real(kind=Rk) xl(nb_quadra)
      real(kind=Rk) d0l(nb_quadra,nb_legendre)
      real(kind=Rk) d1l(nb_quadra,nb_legendre)
      real(kind=Rk) d2l(nb_quadra,nb_legendre)
...
!      valeur en x du polynome en l, l-1 et l-2
      ...
      integer i,l
       ...
       ELSE
         ...
         DO i=2,l
           norme = norme*TWO*real(i,kind=Rk)
           pl0 = TWO*( x*pl1 - real(i-1,kind=Rk)*pl2 )
           pl2 = pl1
           pl1 = pl0
         END DO
...
!===================================================
      ...
      IMPLICIT NONE
       ...
       real(kind=Rk) x,d0,d1,d2

       real(kind=Rk) xp,xm,ep,em,step
       logical num,deriv
       ...
       IF (deriv) THEN
         ...
         ELSE
           ...
           ELSE IF (l == 1) THEN
              ...
              d2 = ZERO
           ELSE
             d1 = sqrt(real(2*l,kind=Rk)) * poly_Hermite(x,l-1)
             d2 = TWO*(x*d1-d0*real(l,kind=Rk))
           END IF
...
!===================================================
      ...
      IMPLICIT NONE
       ...
       real(kind=Rk) x,d0,d1,d2,d3,pexp
...
!----- function -----------------------------------
       ...
       IF (deriv) THEN
         ...
         ELSE
           d1 = sqrt(real(2*l,kind=Rk)) * poly_Hermite(x,l-1)
           d2 = TWO*(x*d1-d0*real(l,kind=Rk))
           d3 = TWO*(x*d2-d1*real(l-1,kind=Rk))
         END IF
...
!===================================================
      ...
      IMPLICIT NONE
       ...
       real(kind=Rk) xh(nb_gauss_h)
       real(kind=Rk) d0h(nb_gauss_h,0:nb_herm)
       real(kind=Rk) d1h(nb_gauss_h,0:nb_herm)
       real(kind=Rk) d2h(nb_gauss_h,0:nb_herm)
       ...
       IF (deriv) THEN
         ...
         ELSE
           ...
           ELSE IF (l == 1) THEN
              ...
              d2 = ZERO
           ELSE
             d1 = sqrt(TWO*real(l,kind=Rk)) * poly_Hermite(x,l-1)
             d2 = TWO*(x*d1-d0*real(l,kind=Rk))
...
!===================================================
      ...
      IMPLICIT NONE
       ...
       real(kind=Rk) xh(nb_gauss_h)
       real(kind=Rk) d0h(nb_gauss_h,nb_herm)
       real(kind=Rk) d1h(nb_gauss_h,nb_herm)
       real(kind=Rk) d2h(nb_gauss_h,nb_herm)
...
FUNCTION poly_laguerre(x,n,a)
  USE UtilLib_m
  IMPLICIT NONE
  real(kind=Rk) :: poly_laguerre

       integer,         intent (in) :: n,a
       real(kind=Rk),intent(in)  :: x

       real(kind=Rk) :: Pp,P0,Pm
...
SUBROUTINE d0d1d2poly_laguerre(x,d0l,d1l,d2l,n,a)
  USE UtilLib_m
  IMPLICIT NONE
  real(kind=Rk) :: d0l,d1l,d2l

       integer,         intent (in) :: n,a
       real(kind=Rk),intent(in)  :: x

       real(kind=Rk) :: Pp,P0,Pm,dx
       real(kind=Rk) :: poly_laguerre ! function
...
SUBROUTINE d0d1d2poly_laguerre_weight(x,d0l,d1l,d2l,n,a)
  USE UtilLib_m
  IMPLICIT NONE
       real(kind=Rk), intent(inout) :: d0l,d1l,d2l
       integer,          intent(in)    :: n,a
       real(kind=Rk), intent(in)    :: x

       real(kind=Rk) :: d0x,d1x,d2x,ra
       real(kind=Rk) :: d0e,d1e,d2e
       real(kind=Rk) :: d0w,d1w,d2w
...
!---------- variables passees en argument ----------------------------
      ...
      real(kind=Rk)  xf(nb_quadra)
      real(kind=Rk)  d0f(nb_quadra,nb_fourier)
      real(kind=Rk)  d1f(nb_quadra,nb_fourier)
      real(kind=Rk)  d2f(nb_quadra,nb_fourier)

      integer ii,k

      integer is,ic
      real(kind=Rk)  xiik,c,s
!---------------------------------------------------------------------
      real(kind=Rk) sq2pi,sqpi
!---------------------------------------------------------------------
...
!         xiik = mod(xf(k)*ii,pi2)
          ...
          d1f(k,ic) = -real(ii,kind=Rk) * s
          d1f(k,is) =  real(ii,kind=Rk) * c

          d2f(k,ic) = -real(ii * ii,kind=Rk) * c
          d2f(k,is) = -real(ii * ii,kind=Rk) * s
        END DO
...
! Taken from http://iris-lee3.ece.uiuc.edu/~jjin/routines/mjynb.for
      ...
      real(kind=Rk) bj(0:nmax)  !  out

                                   !  local
      real(kind=Rk) bs, f2, f1, f, s0, a(4), b(4), a1(4), b1(4),     &
...
!     DATA pi /3.141592653589793/
      ...
      IF (x<= ONETENTH**100) THEN
         bj(0)= ONE
         RETURN
      ENDIF

      nm= nmax
      IF (x<=300_Rk .OR. nmax>=INT(0.9_Rk*x)) THEN
         IF (nmax==0) nm= 1
         m= msta1(x,200)
         IF (m<=nm) THEN
            nm= m
         ...
         f1= tiny(ONE)
         DO k=m, 0, -1
            f= TWO*real(k+1,kind=Rk)/x*f1-f2
            IF (k.LE.nm) bj(k)= f
      ...
      ELSE
         DATA a  /-.0703125000000000_Rk,  .1121520996093750_Rk,   &
                  -.5725014209747314_Rk,  6.074042001273483_Rk /
         DATA b  / .0732421875000000_Rk, -.2271080017089844_Rk,   &
                   1.727727502584457_Rk, -24.38052969955606_Rk /
         DATA a1 / .1171875000000000_Rk, -.1441955566406250_Rk,   &
                   .6765925884246826_Rk, -6.883914268109947_Rk /
         DATA b1 /-.1025390625000000_Rk,  .2775764465332031_Rk,   &
                  -1.993531733751297_Rk,  27.24882731126854_Rk /
         ...
         DO k=2, nm
            bjk= real(2*(k-1),kind=Rk)/x*bj1-bj0
            bj(k)= bjk
            bj0= bj1
            bj1= bjk
         ENDDO
...
! the magnitude of Jn(x) at that point is about 10^(-mp)
      ...
      real(kind=Rk) a0, f0, f1, f  !  local
      INTEGER n, n0, n1, it, nn       !

      real(kind=Rk) envj           !  intrinsic
      envj(n,x)= HALF*log10(6.28_Rk*real(n,kind=Rk)) -            &
              real(n,kind=Rk)*log10(1.36_Rk*x/real(n,kind=Rk))

      a0= abs(x)
      n0= INT(1.1_Rk * a0)+1
      f0= envj(n0,a0)-real(mp,kind=Rk)
      n1= n0+5
      f1= envj(n1,a0)-real(mp,kind=Rk)

      DO it=1, 20
         nn= n1-int(real(n1-n0,kind=Rk)/(ONE-f0/f1))
         f= envj(nn,a0)-real(mp,kind=Rk)
...
! all Jn(x) has mp significant digits
      ...
      real(kind=Rk) a0, hmp, ejn, obj, f0, f1, f  !  local
      INTEGER n0, n1, it, nn                         !
...
!                                  specifications for local variables
      ...
      integer            itemp
      real(kind=Rk)  test,tempa,tempb,tempc,p
      real(kind=Rk)  rsign,sum,tover,plast,pold,psave,psavel
      real(kind=Rk)  dsig,rten,tmpa4,smallx,xinf
      data               dsig/SEVEN/
      data               rten/44._Rk/
      data               xinf/1.7e+38_Rk/
      data               largex /100000/
...
!                                    small arg
      ...
      do 15 nn=2,n
         tempa = tempa*arg/real(2*nn-2,kind=Rk)
         b(nn) = tempa*(ONE+tempb/real(nn,kind=Rk))
   15 continue
      go to 9005
...
!                                    check for possible overflow.
      ...
      do 25 nn=nstart,nend
         pold = plast
         plast = p
         p = real(2*nn,kind=Rk)*plast/tempa-rsign*pold
         if(p-tover) 25, 25, 35
   25 continue
      nn = nend
...
!                                    test is passed.
      ...
      do 45 ncalc=nstart,nend
         pold = psavel
         psavel = psave
         psave = real(2*nn,kind=Rk)*psavel/tempa-rsign*pold
...
!                                    storing) b(nn), until nn=n.
   60 do 65 l=1,nend
         ...
         tempc = tempb
         tempb = tempa
         tempa = real(2*nn,kind=Rk)*tempb/arg-rsign*tempc
         m = 2-m
         sum = sum+real(m,kind=Rk)*tempa
   65 continue
!                                  store b(nn)
   70 b(nn) = tempa
      if(n.gt.1) go to 75
...
!                                  calculate and store b(nn-1)
   75 nn = nn-1
      b(nn) = real(2*nn,kind=Rk)*tempa/arg-rsign*tempb
      if(nn.eq.1) go to 105
      m = 2-m
      sum = sum+real(m,kind=Rk)*b(nn)
      go to 90
...
!                                    store b(nn), until nn=2
      do 95 l=1,nend
         nn = nn-1
         b(nn) = real(2*nn,kind=Rk)*b(nn+1)/arg-rsign*b(nn+2)
         m = 2-m
         sum = sum+real(m,kind=Rk)*b(nn)
   95 continue
!                                  calculate b(1)
...
!      ---------------------------------------------------------------
       ...
       l = int(sqrt(real(i,kind=Rk)-HALF))
       lll = l+1
       mmm = i-l*l
       m = mmm/2
...
!================================================================
      ...
      IMPLICIT NONE
       ...
       real(kind=Rk) d0,d1(2),d2(2,2),x(2)
       integer i
       logical num
       real(kind=Rk) step

       integer l,m,lll,mmm
       real(kind=Rk) th,phi,cth,sth
       real(kind=Rk) d0fm,d1fm,d2fm,d3fm
       real(kind=Rk) d0plm,d1plm,d2plm
...
!      write(out_unitp,*) i,num,step,x,d0,d1,d2
       ...
       ELSE
         d1plm  = real(m,kind=Rk)*cth/sth*d0plm +                    &
               sqrt(real(l+m+1,kind=Rk)*real(l-m,kind=Rk)) *      &
                 poly_legendre(cth,lll,m+1)
         d2plm  = (real(m*m,kind=Rk)/(sth*sth) -                     &
                   real(l*l+l,kind=Rk))*d0plm -                      &
                  cth/sth*d1plm
       END IF
...
!      write(out_unitp,*) i,l,m,d0plm,d1plm,d2plm
     ...
     SUBROUTINE d0d1d2d3Ylm(d0,d1,d2,d3,x,i)
      ...
      IMPLICIT NONE
       ...
       real(kind=Rk) d0,d1(2),d2(2,2),d3(2,2,2),x(2)
       integer i
       logical num
       real(kind=Rk) step
       ...
       real(kind=Rk) th,phi,cth,sth
       real(kind=Rk) d0fm,d1fm,d2fm,d3fm
       real(kind=Rk) d0plm,d1plm,d2plm,d3plm
...
!     ----------------------------------------------------------------
      ...
      IMPLICIT NONE
       ...
       real(kind=Rk) :: th,d0plm,d1plm,d2plm
       integer          :: l,m


       integer          :: lll,mmm
       real(kind=Rk) :: cth,sth
...
!      ---------------------------------------------------------------
       ...
       d1plm  = real(mmm,kind=Rk)*cth/sth*d0plm +                    &
                sqrt(real(l+mmm+1,kind=Rk)*real(l-mmm,kind=Rk)) * &
                poly_legendre(cth,lll,mmm+1)
       d2plm  = (real(mmm*mmm,kind=Rk)/(sth*sth) -                   &
                 real(l*l+l,kind=Rk))*d0plm - cth/sth*d1plm
...
!      write(out_unitp,*) lll,m,d0plm,d1plm,d2plm
      ...
      IMPLICIT NONE
       ...
       real(kind=Rk) :: th,d0plm,d1plm,d2plm,d3plm
       integer          :: l,m


       integer          :: lll,mmm
       real(kind=Rk) :: cth,sth,Rm2,Rllp1
...
!      ---------------------------------------------------------------
       ...
       lll = l+1
       mmm = abs(m)
       Rm2 = real(mmm*mmm,kind=Rk)
       Rllp1 = real(l*l+l,kind=Rk)

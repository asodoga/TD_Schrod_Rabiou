...
!---------------------------------------------------------------------
       ...
       SUBROUTINE d0d1d2fourier(x,d0,d1,d2,i)
        USE QDUtil_m
        IMPLICIT NONE
...
!---------------------------------------------------------------------
       ...
       SUBROUTINE d0d1d2box(x,d0,d1,d2,i)
        USE QDUtil_m
        IMPLICIT NONE
...
!
!===================================================
!      SUBROUTINE d0d1d2poly_Hermite0(x,l,d0,d1,d2,deriv,num,step)
!      USE QDUtil_m
!      IMPLICIT NONE
!
...
!!===================================================
       ...
       SUBROUTINE d0d1d2poly_Hermite_exp(x,l,d0,d1,d2,deriv)
        USE QDUtil_m
        IMPLICIT NONE
...
!      write(out_unit,*) 'l x d0 d1 d2 hermite :',l,x,d0,d1,d2
       ...
       SUBROUTINE d0d1d2poly_Hermite_exp_cplx(x,p,l,d0,d1,d2,deriv)
        USE QDUtil_m
        IMPLICIT NONE
         real(kind=Rkind) x,p
  ...
  ! ----- function -----------------------------------
   ...
   !----- function -----------------------------------
         IF (deriv) THEN
           ...
           !on rajoute la partie exponentielle
           !pour d0 d1 d2, d3
           pexp = exp(-HALF*x*x+EYE*p*x)
           d2 = (d2+TWO*(EYE*p-x)*d1+((EYE*p-x)*(EYE*p-x)-ONE)*d0)*pexp
           d1 = (d1+(EYE*p-x)*d0)*pexp
           d0 = d0*pexp
         ELSE
           d0 = poly_Hermite(x ,l)*exp(-HALF*x*x+EYE*p*x)
           d1 = ZERO
           d2 = ZERO
         END IF
         !write(out_unit,*) 'l x d0 d1 d2 hermite :',l,x,d0,d1,d2
...
!         id = 0
!         DO i=1,nb_herm
!           CALL d0d1d2poly_Hermite_exp1(xh(k),id,                        &
!                     d0h(k,i),d1h(k,i),d2h(k,i),deriv,num,step)
!
!!          write(out_unit,*) i,k,d0h(k,i)
!!          write(out_unit,*) i,k,d1h(k,i)
...
!
!      nm= nmax
!      IF (x<=300_Rkind .OR. nmax>=INT(0.9_Rkind*x)) THEN
!         IF (nmax==0) nm= 1
!         m= msta1(x,200)
!         IF (m<=nm) THEN
!            nm= m
  ...
  !
        ...
        IMPLICIT NONE
         ...
         SUBROUTINE hercom(nq, xp, w)
          Implicit none
          integer         :: i, nq
          real(kind=Rkind)   :: cc, dp2, p1, s, temp, x
          real(kind=Rkind)   :: w(nq), xp(nq)
    
          CC = 1.7724538509_Rkind*gamma_perso(nq)/(TWO**(nq - 1))
    
          S = (TWO*dble(real(nq, Kind=Rkind)) + ONE)**(SIXTH)
    
          DO i = 1, (nq + 1)/2
             IF (i == 1) THEN
                x = s**3 - 1.85575_Rkind/s
             ELSE IF (i == 2) THEN
                x = x - 1.14_Rkind*((dble(nq))**0.426_Rkind)/x
             ELSE IF (i == 3) THEN
                x = 1.86_Rkind*x - 0.86_Rkind*xp(1)
             ELSE IF (i == 4) THEN
                x = 1.91_Rkind*x - 0.91_Rkind*xp(2)
             ELSE
                x = TWO*x - xp(i - 2)
             END IF
       ...
       END SUBROUTINE hercom
        ...
        SUBROUTINE herrec(p2, dp2, p1, x, nq)
          ...
          integer       ::i
          integer       :: nq
          real(kind=Rkind) :: dp0, dp1, dp2, p0, p1, p2, x
       ...
       SUBROUTINE herroot(x, nq, dp2, p1)
          ...
          integer                   :: i
          integer                   :: nq
          real(kind=Rkind), parameter  :: eps = TEN**(-TWELVE) ! 1.0d-12
          real(kind=Rkind)             :: d, dp2, p1, p2, x
